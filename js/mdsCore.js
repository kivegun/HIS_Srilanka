...
function continueAll(prid,prsid,drug_type){
    if (drug_type === 'nowarning') {
        drug_ids = $("#no_warning_prev_drug_ids").val();
    }   else {
        drug_ids = $("#warning_prev_drug_ids").val();
    }
...
function continueThis(prsid,itemid){
	
	$.ajax({        
		type :'POST',
    	url: 'continueThis.php',
    	dataType : 'json',
    	data:{		
    		id:prsid ,
			id2:itemid
...
function printReport(id, type) {
    ...
    } else if (type == 'OSDrugs') {
        url = "osdrugs.php?OPD=" + id;
    ...
    } else if (type == 'ecg_tests') {
        from = arguments[2];
        to = arguments[3];
        url = "ecgtests.php?from=" + from + "&to=" + to;
